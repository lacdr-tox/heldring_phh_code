...
makeCorGenData <- function(genData, withRNA, varyR, mdm2RNA,M033) {
  # Do correlation analysis per set
  if (withRNA & varyR & (!mdm2RNA) & !(M033)) {
    # Option 1
    ...
    corGenData <- genData %>% group_by(run, variation) %>% 
      summarise(
        ...
        CDKN1A_cpt24_vs_TP53_med = cor(x = CDKN1A_cpt24, y = TP53_med),
        BTG2_cpt24_vs_TP53_med = cor(x = BTG2rna_cpt24, y = TP53_med))
  } else if (withRNA & (!varyR) & (!mdm2RNA) &(!M033)) {
    # Option 3
    ...
    # Option 5
    print("corGenData Option 5")
    corGenData <- genData %>% group_by(run, variation) %>% 
      summarise(MDM2_med_vs_TP53_med = cor(x = MDM2rna_med, y = p53rna_total_med),
                CDKN1A_med_vs_TP53_med = cor(x = CDKN1A_med, y = p53rna_total_med),
                BTG2_med_vs_TP53_med = cor(x = BTG2rna_med, y = p53rna_total_med),
                MDM2_cpt8_vs_TP53_cpt8 = cor(x = MDM2rna_cpt8, y = p53rna_total_cpt8),
                CDKN1A_cpt8_vs_TP53_cpt8 = cor(x = CDKN1A_cpt8, y = p53rna_total_cpt8),
                BTG2_cpt8_vs_TP53_cpt8 = cor(x = BTG2rna_cpt8, y = p53rna_total_cpt8),
                TP53_cpt8_vs_TP53_med = cor(x = p53rna_total_cpt8, y = p53rna_total_med),
                MDM2_cpt8_vs_TP53_med = cor(x = MDM2rna_cpt8, y = p53rna_total_med),
                CDKN1A_cpt8_vs_TP53_med = cor(x = CDKN1A_cpt8, y = p53rna_total_med),
                BTG2_cpt8_vs_TP53_med = cor(x = BTG2rna_cpt8, y = p53rna_total_med),
                # 24 hr
                MDM2_cpt24_vs_TP53_cpt24 = cor(x = MDM2rna_cpt24, y = p53rna_total_cpt24),
                CDKN1A_cpt24_vs_TP53_cpt24 = cor(x = CDKN1A_cpt24, y = p53rna_total_cpt24),
                BTG2_cpt24_vs_TP53_cpt24 = cor(x = BTG2rna_cpt24, y = p53rna_total_cpt24),
                TP53_cpt24_vs_TP53_med = cor(x = p53rna_total_cpt24, y = p53rna_total_med),
                MDM2_cpt24_vs_TP53_med = cor(x = MDM2rna_cpt24, y = p53rna_total_med),
                CDKN1A_cpt24_vs_TP53_med = cor(x = CDKN1A_cpt24, y = p53rna_total_med),
                BTG2_cpt24_vs_TP53_med = cor(x = BTG2rna_cpt24, y = p53rna_total_med))
    
  } else if (withRNA & varyR & (!mdm2RNA) & M033) {
    # Option 6
    print("corGenData Option 6")
    corGenData <- genData %>% group_by(run, rfactor, variation) %>% 
      summarise(
        # 8 hr
        MDM2_med_vs_TP53_med = cor(x = MDM2rna_med, y = p53rna_total_med),
        CDKN1A_med_vs_TP53_med = cor(x = CDKN1A_med, y = p53rna_total_med),
        BTG2_med_vs_TP53_med = cor(x = BTG2rna_med, y = p53rna_total_med),
        MDM2_cpt8_vs_TP53_cpt8 = cor(x = MDM2rna_cpt8, y = p53rna_total_cpt8),
        CDKN1A_cpt8_vs_TP53_cpt8 = cor(x = CDKN1A_cpt8, y = p53rna_total_cpt8),
        BTG2_cpt8_vs_TP53_cpt8 = cor(x = BTG2rna_cpt8, y = p53rna_total_cpt8),
        TP53_cpt8_vs_TP53_med = cor(x = TP53_cpt8, y = p53rna_total_med),
        MDM2_cpt8_vs_TP53_med = cor(x = MDM2rna_cpt8, y = p53rna_total_med),
        CDKN1A_cpt8_vs_TP53_med = cor(x = CDKN1A_cpt8, y = p53rna_total_med),
        BTG2_cpt8_vs_TP53_med = cor(x = BTG2rna_cpt8, y = p53rna_total_med),
        # 24 hr
        ...
        CDKN1A_cpt24_vs_TP53_cpt24 = cor(x = CDKN1A_cpt24, y = p53rna_total_cpt24),
        BTG2_cpt24_vs_TP53_cpt24 = cor(x = BTG2rna_cpt24, y = p53rna_total_cpt24),
        TP53_cpt24_vs_TP53_med = cor(x = TP53_cpt24, y = p53rna_total_med),
        MDM2_cpt24_vs_TP53_med = cor(x = MDM2rna_cpt24, y = p53rna_total_med),
        CDKN1A_cpt24_vs_TP53_med = cor(x = CDKN1A_cpt24, y = p53rna_total_med),
        BTG2_cpt24_vs_TP53_med = cor(x = BTG2rna_cpt24, y = p53rna_total_med))
  } else {
    # Option 6
    warning("Option 7 is not provided. No corGenData created.")
  }
  return(corGenData)
}
...
# used for run with 
variableDate <- "20220328_varyR_varyparms" 
modelDate <- "20220326_110349" 
MODEL <- "M033Model_parmset13"
...
mdm2RNA <- F
varyR <- F
# If we measure free mRNA, we set this to False
M033 <- T
...
# Replace name for C_8
colnames(genData_orig)
genData_orig <- genData_orig %>% dplyr::rename(C_24hr = 33)
colnames(genData_orig)
...
# Remove negative initial states
genData <- genData %>% dplyr::select(-contains("noise")) %>% filter_at(vars(matches(c("med","cpt"))), all_vars(. > 0))

# Add p53 rna and MDM2 rna
genData <- genData %>% mutate("p53rna_total_med" = TP53_med + C_med,
                              "p53rna_total_cpt8" = TP53_cpt8 + C_cpt8,
                              "p53rna_total_cpt24" = TP53_cpt24 + C_cpt24,
                              "MDM2_total_med" = MDM2_med + MDM2S395_med + C_med,
                              "MDM2_total_cpt8" = MDM2_cpt8 + MDM2S395_cpt8 + C_cpt8,
                              "MDM2_total_cpt24" = MDM2_cpt24 + MDM2S395_cpt24 + C_cpt24)
...
corGenData <- makeCorGenData(genData, withRNA, varyR, mdm2RNA, M033)
...
ggsave(file = paste0(outputDir_GR,DATE,"_Correlation_Figs/FigSX_genData_boxplot_medium_vs_medium_8_",variableDate,".pdf"),
       width = 5, height = 3)
...
ggsave(file = paste0(outputDir_GR,DATE,"_Correlation_Figs/FigSX_genData_boxplot_cisplatin3.3uM_vs_medium_8_",variableDate,".pdf"),
       width = 5, height = 3)
...
######### ######### ######### ######### ######### ######### 
#### 0b. Check out parameter variations, without noise #######
######### ######### ######### ######### ######### ######### 
...
ggplot() +
  ...
  scale_fill_viridis(discrete = T, name = expression(paste("factor ",italic("c"))),labels = legendlabs)
ggsave(file = paste0(outputDir_GR,DATE,"_Correlation_Figs/FigS11B_genData_boxplot_withNoise",noise,"_medium_vs_medium_8_",variableDate,".pdf"),
       width = 3, height = 3)
...
ggplot() +
  ...
  scale_fill_viridis(discrete = T, name = expression(paste("factor ",italic("c"))),labels = legendlabs) + 
  ggtitle("Cisplatin, 8hr")
ggsave(file = paste0(outputDir_GR,DATE,"_Correlation_Figs/FigS11B_genData_boxplot_withNoise",noise,"_cisplatin3.3uM_vs_medium_8_",variableDate,".pdf"),
       width = 3.5, height = 3)
