...
#load("/data/muriel/Projects/DDP/DataAnalysis/BioSpyder/Exp009_BioSpyder_Marije/DEGAnalysis/Output/20210331workspace.RData")
...
makeCorGenData <- function(genData, withRNA, varyR, mdm2RNA,M033) {
  ...
  if (withRNA & varyR & (!mdm2RNA)) {
    ...
    # Option 6
    warning("Option 6 is not provided. No corGenData created.")
  }
  return(corGenData)
}
...
# used for run with 
variableDate <- "20220329_varyR_MDM2feedback_M034" 
...
# Select only rfactor = 1 and set varyR to False
genData <- genData %>% filter(rfactor == 1) %>% dplyr::select(-c(rfactor))
varyR <- F
...
res_sum_med$axisLab2 <- as.numeric(res_sum_med$name)
ggplot() +
  geom_blank(data = medVSmed %>% filter(variation == 0.1), aes(x = axisLab2, y = value, fill = variation)) +
  geom_hline(aes(yintercept = 0)) +
  geom_rect(data = res_sum_med,
            aes(xmin = axisLab2-0.45,
                xmax = axisLab2+0.45,
  ...
  geom_segment(data = res_sum_med, aes(x=axisLab2-0.45, xend=axisLab2+0.45, y=mean, yend=mean), color = "grey70", linetype = "solid") +
  #  geom_segment(data = hline_med, aes(x=axisLab2-0.45, xend=axisLab2+0.45, y=rho, yend=rho), color = "grey40", linetype = "dashed") +
  geom_boxplot(data = medVSmed %>% filter(variation == 0.1), aes(x = axisLab2, y = value, fill = variation), position = "dodge2", width = 0.7) + 
  theme_classic() + 
  ggtitle("Medium, 8 hr") + 
  ylab("Pearson correlation") + ylim(c(-1,1)) +
  theme(axis.title.x = element_blank(),
  ...
  scale_fill_viridis(discrete = T, name = expression(paste("factor ",italic("c"))),labels = legendlabs)
ggsave(file = paste0(outputDir_GR,DATE,"_Correlation_Figs/FigS11A_genData_boxplot_withNoise",noise,"_medium_vs_medium_8_",variableDate,".pdf"),
       width = 3, height = 3)
...
res_sum_cpt$axisLab2 <- as.numeric(res_sum_cpt$name)
ggplot() +
  geom_blank(data = cptVSmed8 %>% filter(variation == 0.1), aes(x = axisLab2, y = value, fill = variation)) +
  geom_hline(aes(yintercept = 0)) +
  geom_rect(data = res_sum_cpt,
            aes(xmin = axisLab2-0.45,
                xmax = axisLab2+0.45,
  ...
  geom_segment(data = res_sum_cpt, aes(x=axisLab2-0.45, xend=axisLab2+0.45, y=mean, yend=mean), color = "grey70", linetype = "solid") +
  #  geom_segment(data = hline_cpt, aes(x=axisLab2-0.45, xend=axisLab2+0.45, y=rho, yend=rho), color = "grey40", linetype = "dashed") +
  geom_boxplot(data = cptVSmed8 %>% filter(variation == 0.1), aes(x = axisLab2, y = value, fill = variation), position = "dodge2", width = 0.7) + 
  theme_classic() + 
  ylab("Pearson correlation") + ylim(c(-1,1)) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 12),
  ...
  scale_fill_viridis(discrete = T, name = expression(paste("factor ",italic("c"))),labels = legendlabs) + 
  ggtitle("Cisplatin, 8hr")
ggsave(file = paste0(outputDir_GR,DATE,"_Correlation_Figs/FigS11A_genData_boxplot_withNoise",noise,"_cisplatin3.3uM_vs_medium_8_",variableDate,".pdf"),
       width = 3.5, height = 3)
...
res_sum_cpt$axisLab2 <- as.numeric(res_sum_cpt$name)
ggplot() +
  geom_blank(data = cptVSmed24 %>% filter(variation == 0.1), aes(x = axisLab2, y = value, fill = variation)) +
  geom_hline(aes(yintercept = 0)) +
  geom_rect(data = res_sum_cpt,
            aes(xmin = axisLab2-0.45,
                xmax = axisLab2+0.45,
            ...
            fill = "grey", alpha = 0.5) +
  geom_segment(data = res_sum_cpt, aes(x=axisLab2-0.45, xend=axisLab2+0.45, y=mean, yend=mean), color = "grey70", linetype = "solid") +
  geom_boxplot(data = cptVSmed24 %>% filter(variation == 0.1), aes(x = axisLab2, y = value, fill = variation), position = "dodge2", width = 0.7) + 
  theme_classic() + 
  ylab("Pearson correlation") + ylim(c(-1,1)) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 12),
